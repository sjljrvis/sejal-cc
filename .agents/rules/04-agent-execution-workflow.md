...
```
`npm run build` runs the TypeScript compiler — a green build means no type errors. Do not skip it for "small" changes; type errors love hiding in trivial diffs.

**Backend:**
```bash
make lint       # ruff (and mypy if configured) — fix all errors and obvious warnings
make test-be    # full pytest suite for affected modules
```
A bare `python -m py_compile` is **not** sufficient — it only catches syntax errors, not type, import, or behavioral issues.

**Migrations:** If you changed any model in `app/models/`, you MUST also have run `make migrate-create` and `make migrate-up` and verified the chain with `make migrate-history` before committing. See §6.

If any check fails, read the errors and fix them before committing. **NEVER leave the codebase in a broken state.**
